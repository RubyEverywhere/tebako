...
When an application has a `Gemfile.lock`, Tebako caches its installed gems under
the Tebako prefix. Later presses restore the matching Ruby/platform dependency
set and use `bundle check`, running `bundle install` only when gems are missing.
...
In `both` mode, the application package is split into immutable application,
executable, and gem filesystem layers. An application-only edit recompresses the
application layer while reusing unchanged gem and executable layers. The
matching runtime mounts those layers at startup. `application` mode continues
to emit a monolithic image so it remains compatible with previously built
runtime executables.
...
runtime validation result. Keep the Ruby version, compression level, prefix,
machine load, and application fixed when comparing revisions.
