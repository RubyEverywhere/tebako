...
[source,sh]
----
tebako runtime build \
  --Ruby=4.0.6 \
  --output=build/tebako-runtime \
  --sdk-output=build/tebako-runtime-sdk.tar.gz

tebako runtime install build/tebako-runtime-sdk.tar.gz \
  --destination=$HOME/.tebako/runtime-sdk

tebako application build \
  --runtime=build/tebako-runtime \
  --root=. \
...
SDK installation verifies the archive manifest and file checksums before
atomically replacing an existing installation. Tebako also exposes cache and
package diagnostics:
...
tebako cache stats
tebako cache verify
tebako cache prune --max-size=20GB
tebako package inspect build/my-app.tebako
tebako package verify build/my-app.tebako
----
